...
import { getArchiveBindings, ensureArchiveSchema, jsonError, localOcrServiceUrl, resolveOriginalObject } from "../../../../lib/archive-storage";
...
async function releaseFailedJob(db: D1Database, job: ClaimedJob, message: string) {
  ...
  await db.prepare(`UPDATE processing_jobs SET status = ?, error_message = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
    .bind(status, message.slice(0, 1000), job.id).run();
...
export async function POST(request: Request) {
  ...
  const job = await bindings.DB.prepare(`UPDATE processing_jobs
    ...
    RETURNING id, document_id, attempt, max_attempts`).bind(requestedDocumentId, requestedDocumentId, requestedDocumentId, principal.unit, principal.unit).first<ClaimedJob>();

  if (!job) return Response.json({ processed: false, message: "Bekleyen OCR işi yok." });
  ...
  try {
    ...
    if (!document) throw new Error("OCR işine ait belge bulunamadı.");
    const original = await resolveOriginalObject(bindings.DB, document.id);
    if (!original) throw new Error("Belgenin asıl nesne kaydı bulunamadı.");
    const object = await bindings.ARCHIVE_FILES.get(original.object_key);
    if (!object) throw new Error("Asıl dosya kasada bulunamadı.");
    ...
    for (const page of result.pages) {
      writeStatements.push(bindings.DB.prepare(`INSERT INTO ocr_pages
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .bind(crypto.randomUUID(), document.id, page.pageNumber, page.width, page.height, page.rawText, page.fullText, page.searchText, JSON.stringify(page.words), page.averageConfidence, result.model));
    }
    for (const field of fields) {
    ...
    await bindings.DB.batch([
      ...
      bindings.DB.prepare(`UPDATE processing_jobs SET status = 'completed', model = ?, error_message = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
    ...
    return Response.json({
      ...
      profileVersion: effectiveProfile.profileVersion, auditEvent: audit.eventNumber,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "OCR işlemi tamamlanamadı.";
    await releaseFailedJob(bindings.DB, job, message);
