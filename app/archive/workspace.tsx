...
type Overview = {
  ...
  storage:{ objects:number; bytes:number; legacyKeys:number; withoutAccessDerivative:number;
    /** Kota küresel ölçülür; tavan tanımlı değilse configured:false gelir. */
    quota?:{ configured:boolean; usedBytes:number; limitBytes?:number; usedRatio?:number } };
...
{/* Servis sağlığı ölçülür (Ayarlar ekranıyla aynı kaynak); yedekleme ve
        ...
        {/* Kota tavanı İş Etki Analizi kararıdır; tanımsızken uydurma oran gösterilmez. */}
        <div className="backup"><Clock3 size={17}/><span>Kapasite kotası{(()=>{const kota=overview?.storage.quota;
          if(!kota) return <b>Ölçülüyor…</b>;
          if(!kota.configured) return <b>Tanımlı değil</b>;
          const oran=Math.round((kota.usedRatio??0)*100);
          return <b className={oran>=80?"pending":""}>{`%${oran} dolu · ${formatBytes(kota.usedBytes)} / ${formatBytes(kota.limitBytes??0)}`}</b>;
        })()}</span></div>
      </article>
    </section>
...
export function ArchiveWorkspace(){
  ...
  return <div className={`archive-app ${dark?"dark":""}`}><aside className={`sidebar ${mobile?"open":""}`}><div className="brand">{/* eslint-disable-next-line @next/next/no-img-element -- Kurum amblemi statik varlıktır; iyileştirme katmanı gerekmez. */}
    ...
    {selectedId
      /* Aramadan açılan belgede aranan kelimenin yeri işaretlenir; sorgu bu
         yüzden inceleme ekranına taşınır. Arama yokken boş geçer. */
      ?<DocumentReview documentId={selectedId} onBack={backToList} permissions={user?.permissions??[]} searchTerm={searching?query.trim():""}/>
